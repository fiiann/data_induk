...
<?php
	...
	if(!isset($_POST['edit'])){
		...
		if ($validNama && $validAlamat && $validKota && $validEmail && $validTlp) {
			...
			$query = "UPDATE anggota SET nama='".$nama."', alamat='".$alamat."',
			kota='".$kota."', email='".$email."', no_telp='".$noTlp."' WHERE nim='".$nimlawas."'";
					...
					<div class="col-md-12">
						<form method="POST" role="form" autocomplete="on" action="">
							<div class="form-group" hidden>
								<label>NIM</label>
								<input class="form-control" type="text" name="nim" maxlength="14" size="30" placeholder="nim 14 digit angka"  autofocus value="<?php if(isset($nim)) echo $nim; else echo $_POST['nim_new']; ?>" />
							</div>
							<div class="form-group">
								<label>NIM</label>&nbsp;* <span class="label label-warning"><?php if(isset($errorNim)) echo $errorNim;?></span>
								<input class="form-control" type="text" disabled name="nim_new" maxlength="14" size="30" placeholder="nim 14 digit angka" required autofocus value="<?php if(isset($nim)) echo $nim; else echo $_POST['nim_new']; ?>">
							</div>
							<div class="form-group">
